...
import sqlite3
import contextlib
from forensicsUI.helpers import convert_string_to_list, convert_list_to_string, convert_bbox
...
class DatabaseManager:
    ...
    def connect_db(self):
        os.makedirs("forensicsUI/database", exist_ok=True)
        # path = "forensicsUI"
        db_conn = DatabaseConnection(f"forensicsUI/{self.db_name}.db")
        try:
            yield db_conn
        finally:
            db_conn.close()
